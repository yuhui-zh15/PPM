...
const static int kTeapotNumPatches = 32;//32个Bezier曲面，每个曲面16个控制点
const static int kTeapotNumVertices = 306;//306个控制点，因为有重复
...
};//这个表格对应下方数据index减一，告诉我们每个曲面的点序号！
...
Vector3d** createTeapot() {
    Vector3d** ret = new Vector3d*[32];
    Matrix3d change;
    int lambda = 7;
    change(0) = cos(0.4); change(1) = -sin(0.4); change(2) = 0; 
    change(3) = sin(0.4); change(4) = cos(0.4); change(5) = 0;
    change(6) = 0; change(7) = 0; change(8) = 1;
    for (int i = 0; i < 32; i++) {
        ret[i] = new Vector3d[16];
        for (int j = 0; j < 16; j++) {
            ret[i][j](0) = teapotVertices[teapotPatches[i][j] - 1][0];
            ret[i][j](1) = teapotVertices[teapotPatches[i][j] - 1][2];
            ret[i][j](2) = teapotVertices[teapotPatches[i][j] - 1][1];
            ret[i][j] = change * ret[i][j] * lambda;
        } 
...
Vector3d* createDrop() {
    ...
    int coeff = 8;
    ret[0] = Vector3d(0, 1, 0) * coeff;
    ret[1] = Vector3d(0, 1, 0) * coeff;
    ret[2] = Vector3d(0, 1, 0) * coeff;
    ret[3] = Vector3d(0, 1, 0) * coeff;
    ret[4] = Vector3d(0, 1, 0) * coeff;
    ret[5] = Vector3d(0, 0, 1.5) * coeff;
    ret[6] = Vector3d(1, 0, -0.5) * coeff;
    ret[7] = Vector3d(0, 1, 0) * coeff;
    ret[8] = Vector3d(0, 1, 0) * coeff;
    ret[9] = Vector3d(-1, 0, -0.5) * coeff;
    ret[10] = Vector3d(0, 0, 1.5) * coeff;
    ret[11] = Vector3d(0, 1, 0) * coeff;
    ret[12] = Vector3d(0, 1, 0) * coeff;
    ret[13] = Vector3d(0, 1, 0) * coeff;
    ret[14] = Vector3d(0, 1, 0) * coeff;
    ret[15] = Vector3d(0, 1, 0) * coeff;
    return ret;
}
